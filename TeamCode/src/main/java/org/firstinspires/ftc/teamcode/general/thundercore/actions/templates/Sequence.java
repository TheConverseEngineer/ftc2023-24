 ...
 * to get a command that, when run, will stop the sequence it is currently in (assuming one exists)
 *
...
public final class Sequence extends Action {
    ...
    /** Use this command to stop a sequence or loop iteration prematurely if and only if the given condition is true
     * Note that when used in a loop, it has the same effect as the {@code continue} keyword.
     *
     * @return  A command that, when called, will stop the active sequence.
     * */
    public static Action exitSequenceIf(BooleanSupplier condition) {
        return new ExitSequenceActionIf(condition);
    }
    ...
    /** Dummy action that will terminate a sequence when called if a given condition is true */
    public static final class ExitSequenceActionIf extends Action {
        private final BooleanSupplier condition;

        public ExitSequenceActionIf(BooleanSupplier condition) {
            this.condition = condition;
        }

        @Override
        public ACTION_KEYWORD getActionKeyword() {
            return (condition.getAsBoolean()) ? ACTION_KEYWORD.EXIT_SEQUENCE : ACTION_KEYWORD.NONE;
        }
