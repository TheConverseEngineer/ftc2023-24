...
class Trajectory(
    ...
    /** This method returns the trajectory target at some position
     *
     * The returned movement vector is in global space
     */
    fun getGlobalRobotTargetAtTime(t: Double) : TrajectoryState {
        ...
        return TrajectoryState(
            point.getVector(),
            tangentUnitVector*v + normalUnitVector*(mecConfig.Kc*v*v*curvature),
...
class ProfilePointImpl (
    ...
    init {
        ...
        val tangentUnitVector = Vector2d(point.x.dx, point.y.dx)/tangentMag.x
        val normalUnitVector  = Vector2d((differentialShift(point.x)/tangentMag).dx, (differentialShift(point.y)/tangentMag).dx)/normalMag.x
        val directionVector   = Vector2d.createForwardVector(robotHeading)

        velocity = unboundedBinarySearch(
            {v: Double ->
                val globalPower = tangentUnitVector*v + normalUnitVector*(mecConfig.Kc*v*v*curvature)
                val localPower  = Vector2d.convertToLocal(globalPower, directionVector)
                (abs(localPower.y) + abs(localPower.x)*mecConfig.Kl + abs(mecConfig.drivetrainMu*omega*v)) > mecConfig.maxWheelSpeed
            }, precision)
        val globalPower = tangentUnitVector*velocity + normalUnitVector*(mecConfig.Kc*velocity*velocity*curvature)
        val localPower  = Vector2d.convertToLocal(globalPower, directionVector)
