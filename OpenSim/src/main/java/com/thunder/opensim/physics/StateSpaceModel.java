...
public abstract class StateSpaceModel {
    ...
    public void assignMotors(String[] names) {
        System.out.println("Assigned " + names.length + " motors to subsystem");
        this.motors = names;
    }
    ...
    protected final double getRawInputPower() {
        double sum = 0;
        for (String name: motors)
            sum += VirtualHardwareBridge.getInstance().getMotorPower(name);
        System.out.println("Raw power: " + (sum/motors.length));
    ...
    public void update(double dTime) {
        ...
        currentState.set(currentState.a + dX.a*dTime + 0.5*dX.b*dTime*dTime, dX.a + dX.b*dTime);
        lastRecordedAccel = dX.b;

        System.out.println("Updated system: " + currentState.a + " " + currentState.b);

        VirtualHardwareBridge bridge = VirtualHardwareBridge.getInstance();
        for (String name : motors) {
            bridge.setMotorPos(name, Math.round((float) currentState.a));
        }
    }
